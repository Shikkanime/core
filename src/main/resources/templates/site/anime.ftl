...
<@navigation.display canonicalUrl=canonicalUrl openGraphImage="${apiUrl}/v1/attachments?uuid=${anime.uuid}&type=BANNER">
    <div class="anime-card">
        <img src="${apiUrl}/v1/attachments?uuid=${anime.uuid}&type=BANNER"
             alt="${animeSanitized} anime" class="anime-banner">
        ...
        <div class="anime-overlay">
            <div class="tags">
                <#list anime.langTypes as langType>
                    <span class="tag"><@langTypeComponent.display langType=langType /></span>
                </#list>
                <#list anime.simulcasts as simulcast>
                    <a href="/catalog/${simulcast.slug}" class="tag">${simulcast.label}</a>
                </#list>
            </div>

            <h1 class="title">${animeSanitized}</h1>
            <p class="description">${(anime.description!"Aucune description pour le moment...")?html}</p>
        </div>
    </div>
