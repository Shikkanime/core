...
object AnimationDigitalNetworkWrapper : AbstractAnimationDigitalNetworkWrapper() {
    private val headers = mapOf("X-Source" to "Web")

    override suspend fun getLatestVideos(date: LocalDate): Array<Video> {
        val response = httpRequest.get("${baseUrl}video/calendar?date=$date", headers)
    ...
    override suspend fun getShow(id: Int): Show {
        val response = httpRequest.get("${baseUrl}show/$id?withMicrodata=true", headers)
    ...
    override suspend fun getShowVideos(id: Int): Array<Video> {
        val response = httpRequest.get("${baseUrl}video/show/$id?order=asc&limit=-1", headers)
    ...
    override suspend fun getVideo(id: Int): Video {
         val response = httpRequest.get("${baseUrl}video/$id/public", headers)
