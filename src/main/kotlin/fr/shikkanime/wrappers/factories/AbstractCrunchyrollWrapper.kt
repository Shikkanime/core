...
abstract class AbstractCrunchyrollWrapper {
    ...
    suspend fun retrievePreviousEpisode(locale: String, id: String): BrowseObject? {
        // Fetch previous episode using episode discover type
        runCatching { getEpisodeDiscoverByType(locale, "previous_episode", id) }.getOrNull()?.let { return it }
    ...
    suspend fun retrieveNextEpisode(locale: String, id: String): BrowseObject? {
        // Fetch next episode using episode discover type
        runCatching { getEpisodeDiscoverByType(locale, "up_next", id) }.getOrNull()?.let { return it }
