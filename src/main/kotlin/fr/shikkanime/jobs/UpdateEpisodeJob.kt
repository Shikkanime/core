...
class UpdateEpisodeJob : AbstractJob {
    ...
    override suspend fun run() {
        ...
        val lastImageUpdateDateTime = zonedDateTime.minusDays(
            configCacheService.getValueAsInt(ConfigPropertyKey.UPDATE_IMAGE_EPISODE_DELAY, 2).toLong()
        )
        ...
        needUpdateEpisodes.forEach { episodeMapping ->
            ...
            val mappingIdentifier = "${StringUtils.getShortName(episodeMapping.anime!!.name!!)} ${
                StringUtils.toEpisodeMappingString(episodeMapping)
            }"
            val shouldIgnoreCache = attachmentService.findByEntityUuidTypeAndActive(
                episodeMapping.uuid!!,
                ImageType.BANNER
            )?.url == Constant.DEFAULT_IMAGE_PREVIEW && episodeMapping.releaseDateTime.isAfterOrEqual(
            ...
            episodeVariants.forEach { episodeVariant ->
                ...
                val predicate: (AbstractPlatform.Episode) -> Boolean =
                    { it.getIdentifier() == episodeVariant.identifier }
    ...
    private suspend fun fetchNeighbors(
        ...
        val checkPreviousAndNextEpisodes =
            configCacheService.getValueAsBoolean(ConfigPropertyKey.CHECK_PREVIOUS_AND_NEXT_EPISODES)
