...
class MapCache<K : Any, V : Serializable>(
    ...
    private val cacheStrategy = if (!SynchronizedGlideClient.isAvailable()) InMemoryCacheStrategy<K, MapCacheValue<V>>() else ValkeyCacheStrategy<K, MapCacheValue<V>>(name, serializationType, typeToken)
    ...
    override fun removeInvalidated() {
        val now = System.currentTimeMillis()
        cacheStrategy.removeIfValue { value -> now - value.timestamp > duration.toMillis() }
    }
