...
class ValkeyCacheStrategy<K, V>(
    ...
    private fun getWithKeyString(key: String): V? = getCachedValue(key)?.let { SerializationUtils.deserialize(serializationType, it, typeToken) }
    ...
    override fun removeIfValue(filter: (V) -> Boolean) {
        val filteredKeys = keys().filter { key -> filter(getWithKeyString(key)!!) }

        if (filteredKeys.isNotEmpty())
            SynchronizedGlideClient.del(filteredKeys.toTypedArray())
    }
