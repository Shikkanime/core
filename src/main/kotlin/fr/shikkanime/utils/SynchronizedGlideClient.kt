...
object SynchronizedGlideClient {
    ...
    operator fun get(key: String): String? = synchronized(this) {
        glideClient.getOrNull()?.get(key)?.get()?.takeIf { it.isNotBlank() }
    }

    operator fun set(key: String, value: String) = synchronized(this) {
        glideClient.getOrNull()?.set(key, value)?.get()
    }

    fun del(keys: Array<String>) = synchronized(this) {
        glideClient.getOrNull()?.del(keys)?.get()
    }

    fun search(pattern: String, count: Long = 1000): List<String> = synchronized(this) {
        glideClient.getOrNull()?.scan("0", ScanOptions.builder().matchPattern(pattern).count(count).build())?.get()
