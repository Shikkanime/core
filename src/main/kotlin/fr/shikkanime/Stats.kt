...
fun main() {
    ...
    val animes = animeService.findAllLoaded()
    ...
    val previousDubbedAnimes = previousAnimes.filter { episodeVariantService.findAudioLocalesAndSeasonsByAnime(it).first.contains("fr-FR") }.size
    ...
    val actualPlatformCount = actualAnimes.flatMap { episodeVariantService.findAllByAnime(it) }
        .groupBy { it.platform!! }
        .mapValues { (_, value) -> value.map { it.mapping!!.anime!! }.distinctBy { it.uuid }.size }
    val actualDubbedAnimes = actualAnimes.filter { episodeVariantService.findAudioLocalesAndSeasonsByAnime(it).first.contains("fr-FR") }.size
